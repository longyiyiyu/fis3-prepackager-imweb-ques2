...
p._init = function(cnt, file, ret, settings) {
    ...
    this.getComp = settings.getComp;

    this.$ = cheerio.load(cnt);
    // this page's custom targets
    this.customTags = new TagSet();
    this.childComponentsTags = new TagSet();
    this.thirdComponetsTags = new TagSet();
...
p.getMainScript = function() {
    ...
    if (!this.mainScript) {
        ...
        arr.forEach(function(id) {
            if (self.mainScript || self.settings.libs && ~self.settings.libs.indexOf(id)) {
                return;
            }
            self.mainScript = id;
        });
...
p.buildTag = function() {
    ...
    var childComponentsTags = this.childComponentsTags;
    var thirdComponetsTags = this.thirdComponetsTags;
    ...
    var hasChild = false;
    var hasThird = false;
    // var extendMap = {}; // 做不了extend，因为extend是各自处理的，这里统筹处理不了，要另想方案
    ...
    tags.custom().forEach(function(customTag) {
        ...
        if (comp) {
            ...
            if (/^diy-/.test(customTag)) {
                mod = fis.util(comp.root, comp.dir, 'main');
                // clear module cache
                ...
                self.file.addRequire(fis.util(comp.dir, 'main.css').replace(/^\//, ''));
                // self.appendCSS(customTag);

                // console.log('aaaa', comp.html, self.ret.src[comp.html]);
                fragmentTpl = new Tag(self.ret.src[comp.html].getContent(), customTag, l.extend({}, self.settings, {
                    ret: 'function'
                })).tpl;
                $customTag = $(customTag);

                if (!self.settings.noReplaceHtml) {
                ...
                // console.log('test', comp, comp.html in self.ret.src);
                tag = new Tag(self.ret.src[comp.html].getContent(), customTag, l.extend({}, self.settings, {
                    ret: 'function'
                }));
                fragmentTpl = tag.tpl;
                $customTag = $(customTag);
                ...
                tag.dependences.values()
                    .forEach(function(dep) {
                        ...
                        if (!/^ui-/.test(dep) && !/^third-/.test(dep)) {
                            childComponentsTags.add(dep);
                        }

                        if (/^third-/.test(dep)) {
                            thirdComponetsTags.add(dep);
                        }

                        if (/^ui-/.test(dep)) { // ui 组件需要添加css依赖
                            c = self.getComp(dep);
                            self.file.addRequire(fis.util(c.dir, 'main.css').replace(/^\//, ''));
                        }
                    });
    ...
    if (!this.mainScript) {
        ...
        cnt = ['(function(){', "var f = require('comFactory');", 'var c;'];
        hasChild = childComponentsTags.values().length;
        hasThird = thirdComponetsTags.values().length;
        if (hasChild) {
            cnt.push("var Q = require('Q');");
            childComponentsTags.values().forEach(function(dep) {
                var c = self.getComp(dep);
                cnt.push("Q.define('" + dep + "', require('" + c.js.replace(/\.js$/, '') + "'));");
            });
        }
        if (hasThird) {
            cnt.push("var Third = require('third');");
            thirdComponetsTags.values().forEach(function(dep) {
                var c = self.getComp(dep);
                cnt.push("Third.key('" + dep + "', require('" + c.js.replace(/\.js$/, '') + "'));");
            });
        }
        deps.forEach(function(customTag, index) {
            var c = self.getComp(customTag);
            // console.log('insert id [',id,'] to mainScript');
            cnt.push("c = require('" + c.js.replace(/\.js$/, '') + "');");
